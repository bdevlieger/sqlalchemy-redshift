...
def test_defined_types():
    ...
    assert sqlalchemy_redshift.dialect.DOUBLE_PRECISION is sqlalchemy.dialects.postgresql.DOUBLE_PRECISION

    assert sqlalchemy_redshift.dialect.TIMESTAMPTZ is not sqlalchemy.sql.sqltypes.TIMESTAMP


def test_custom_type():
    timestamptz = sqlalchemy_redshift.dialect.TIMESTAMPTZ()
    assert isinstance(timestamptz, sqlalchemy.sql.sqltypes.TIMESTAMP)
    compiler = sqlalchemy_redshift.dialect.RedshiftDDLCompiler(sqlalchemy_redshift.dialect.RedshiftDialect(), None)

    table = sqlalchemy.Table('t1',
                sqlalchemy.MetaData(),
                sqlalchemy.Column('id', sqlalchemy.INTEGER, primary_key=True),
                sqlalchemy.Column('name', sqlalchemy.String),
                sqlalchemy.Column('created_at', sqlalchemy_redshift.dialect.TIMESTAMPTZ))
